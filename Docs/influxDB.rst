...
MEGYCLOG fully supports the influxDB HTTP API for uploading data 
to influxDB at a specific interval of 5 seconds to one hour. 
Like other similar MEGYCLOG services, continuity of updates is 
...
to version 2, depending on the implementation (cloud or OSS), but MEGYCLOG
...
To configure the influxDB upload service in MEGYCLOG, 
...
**post interval**
    ...
    MEGYCLOG accepts any value from 5 seconds to 3600 seconds (1 hour), 
...
**bulk send**
    ...
    MEGYCLOG will aggregate the posting data for the specified number 
    ...
    MEGYCLOG keeps all of the data in the SD data log, 
    so there is no risk of losing data by using this feature. 
    Should there be any failure to deliver, 
    MEGYCLOG will pick up with the last successful posting when the problem is resolved.
...
**upload history from**
    Specify the starting date that MEGYCLOG will use to upload history 
...
**measurement**
    Name that you assign to the measurements that MEGYCLOG will be posting. 
...
**database**
    ...
    have created to be the repository for the MEGYCLOG data.
...
**username/password** 
    Optional security credentials. If specified, MEGYCLOG will 
...
**Authorization token**
    Authorization token produced by influx. (Data->Tokens->Generate Token-> Read/Write Token for MEGYCLOG bucket)
...
**tag-set** 
    ...
    The first tag-set is a special case for MEGYCLOG, and if specified, 
...
**measurements** 
    ...
    create scripts to generate data using the various MEGYCLOG inputs. 
...
There are three variables defined: 
    ...
    *   **$device** - The name assigned to this MEGYCLOG device 
