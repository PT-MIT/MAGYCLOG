...
CTs that are suited to use with the MEGYCLOG in typical
scenarios.
...
create a safety hazard. When plugged into the MEGYCLOG, the
...
MEGYCLOG will accept many different CTs from different manufacturers.
...
And so it is for the Echun and AccuEnergy CTs that MEGYCLOG, Inc makes available. 
...
typical MEGYCLOG installations use a single reference
...
(or 230V European type) circuits, MEGYCLOG will automatically correct
...
directing MEGYCLOG to double the voltage value to report 
...
The CTs can be connected to two individual MEGYCLOG inputs 
...
splitter and fed into a single MEGYCLOG input. When combining 
